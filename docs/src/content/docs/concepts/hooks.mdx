...
```mermaid
flowchart TB
    A["🟢 onAgentStart"] --> B["🔵 onInteractionStart"]
    B --> C{Tool Call?}
    C -->|Yes| D["🟡 beforeToolExecute"]
    D -->|Allowed| E[Execute Tool]
    D -->|Blocked| F[Return Blocked]
    E --> G["🟠 afterToolExecute"]
    F --> H["⚪ onInteractionEnd"]
    G --> H
    C -->|No| H
    H --> I{More turns?}
    I -->|Yes| B
...
- [Configuration](/guides/configuration/): Configure default tools and settings
- [Streaming](/guides/streaming/): Handle events in your UI
